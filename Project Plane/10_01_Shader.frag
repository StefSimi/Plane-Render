...
uniform int isCloud; // 1 if rendering a cloud, 0 otherwise
...
void main(void)
  {
	...
	if (isCloud == 1) {
        out_Color = vec4(result, 0.5f); // Clouds with transparency
    } else {
        out_Color = vec4(result, 1.0f); // Fully opaque for other objects
    }
    }